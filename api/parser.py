    ...
    def _get_details(self, classname: str) -> None:
        ...
        try:
            ...
            for i in all_details:
                ...
                # Check if the title is "Original Network" and process it accordingly
                if _title == "Original Network:":
                    # Find all <a> tags within the <li> and get their text
                    networks = [a.text for a in i.find_all("a", class_="text-primary")]
                    # Join the network names with ', '
                    self.info["details"][_title.replace(":", "").replace(" ", "_").lower()] = ", ".join(networks)
                else:
                    # For other details, handle as before
                    self.info["details"][
